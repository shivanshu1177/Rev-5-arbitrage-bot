...
int main(int argc, char** argv) {
    ...
    arb::LiveOrderHandler handler{&sender, buy_templates, sell_templates};
    ...
    std::thread ws_thread([&]() {
        ...
        ws.setOnMessageCallback([&](const ix::WebSocketMessagePtr& msg) {
            if (msg->type == ix::WebSocketMessageType::Open) {
                ...
                // Timestamp set once, after parse, using local monotonic clock.
                // Both venues share the same clock source for consistent staleness comparison.
    ...
    auto engine_loop = [&](auto& h) {
        ...
        while (g_running.load(std::memory_order_relaxed)) {
            ...
            // Check stop deadline once per 65536 iterations — avoids steady_clock
            // overhead in tight spin and keeps latency measurements clean.
    ...
    spdlog::info("Shutdown signal received. Joining WS thread...");
